...
SPREAD_PIPS = {
    'EURUSD=X': 0.00012, # 1.2 pips
    'GBPUSD=X': 0.00015, # 1.5 pips
    'USDCAD=X': 0.00016, # 1.6 pips
    'USDJPY=X': 0.014    # 1.4 pips
}
COMMISSION_PER_LOT = 3.50 # $3.50 / lot ($7.00 round turn)

class HighPerformanceMultiPairSimulator:
    def __init__(self, ticker, initial_balance=1000.0, base_lot=0.01, max_layers=4, shield_pct=4.0):
        self.ticker = ticker
        ...
        # HIGH WIN RATE + HIGH FREQUENCY OPTIMIZED PARAMETERS
        # Risk/Reward tuned for ~70-80% Win Rate with 2x-3x higher trade frequency
        self.tp_pips = 20.0             # Achievable quick profit target
        self.sl_pips = 16.0             # Tight risk control
        self.be_pips = 8.0              # Quick break even trigger to eliminate risk
        self.be_lock_pips = 2.0         # Locked profit pips
        self.trail_start_pips = 12.0    # Active profit trailing
        self.trail_step_pips = 4.0
        self.grid_step_pips = 14.0      # Active recovery layers
    ...
    def fetch_m15_data(self):
        # Fetch 60d of 15m interval data from Yahoo Finance for granular active trading
    ...
    def calculate_indicators(self, df):
        close = df['Close']
        df['EMA_Fast'] = close.ewm(span=8, adjust=False).mean()   # Fast 8 EMA for responsive triggers
        df['EMA_Slow'] = close.ewm(span=21, adjust=False).mean()  # 21 EMA signal line
        df['EMA_Trend'] = close.ewm(span=50, adjust=False).mean() # 50 EMA for flexible trend alignment
    ...
    def run_monthly_simulation(self):
        ...
        for ym, month_df in df.groupby('YearMonth'):
            ...
            for idx, row in month_df.iterrows():
                ...
                high  = row['High']
                low   = row['Low']
                atr   = row['ATR'] if not np.isnan(row['ATR']) else (14.0 * self.pip_size)
                ...
                if not self.shield_hit and len(self.positions) == 0:
                    ...
                    # Active session hours (06:00 to 20:00 UTC) covering London & New York
                    if 6 <= hour <= 20:
                        ema_f = row['EMA_Fast']
                        ema_s = row['EMA_Slow']
                        ema_t = row['EMA_Trend']
                        ...
                        if prev_idx >= 0:
                            ...
                            # Responsive Momentum Signals on Closed Candle
                            buy_sig  = (ema_f > ema_s) and (prev_ema_f <= prev_ema_s) and (close > ema_t) and (rsi > 48.0)
                            sell_sig = (ema_f < ema_s) and (prev_ema_f >= prev_ema_s) and (close < ema_t) and (rsi < 52.0)
    ...
    def manage_positions(self, close, high, low, atr, month_history):
        ...
        grid_dist = max(self.grid_step_pips * self.pip_size, 1.0 * atr)
...
if __name__ == '__main__':
    ...
    print("=" * 110)
    print("      NUEVA CONFIGURACIÓN DE ALTA FRECUENCIA Y RENTABILIDAD ELEVADA (ROBOFOREX / LIBERTEX ECN)")
    print("      Spreads Reales: 1.2 - 1.6 pips | Comisiones: $3.50/lote | Ticks Reales M15 | Shield 4%")
    print("=" * 110)
    ...
    for pair in pairs:
        ...
        sim = HighPerformanceMultiPairSimulator(ticker=pair, initial_balance=1000.0)
        m_reports = sim.run_monthly_simulation()

        print(f"\n>>> DIVISA / PAR: {pair_clean} (Capital Inicial: $1,000 USD | Presets Optimizados)")
        print("-" * 110)
        ...
        ret_pct = (tot_net / 1000.0) * 100.0
        print(f"RESUMEN {pair_clean}: Beneficio Neto: {tot_sign}${tot_net:.2f} ({tot_sign}{ret_pct:.2f}%) | Trades Totales: {tot_tr} | Win Rate Promedio: {np.mean([r['win_rate'] for r in m_reports]):.1f}%")
        print("=" * 110)

        tot_port_initial += 1000.0
        tot_port_final += (1000.0 + tot_net)
        total_trades_all += tot_tr

    print("\n" + "=" * 110)
    net_port = tot_port_final - tot_port_initial
    p_sign = "+" if net_port >= 0 else ""
    p_pct = (net_port / tot_port_initial) * 100.0
    print(f"🏆 PORTAFOLIO MULTI-PAR CONSOLIDADO (4 DIVISAS SIMULTÁNEAS):")
    print(f"Capital Inicial Global: ${tot_port_initial:.2f} USD | Capital Final Global: ${tot_port_final:.2f} USD")
    print(f"BENEFICIO NETO TOTAL: {p_sign}${net_port:.2f} USD ({p_sign}{p_pct:.2f}%) | OPERACIONES TOTALES: {total_trades_all} trades")
    print("=" * 110)
