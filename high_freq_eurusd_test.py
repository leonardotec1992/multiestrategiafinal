...
SPREAD_PIP = 0.00012 # 1.2 pips
COMMISSION_PER_LOT = 3.50 # $3.50 / lot ($7.00 round turn)

class HighFrequencyEURUSDBacktester:
    def __init__(self, initial_balance=1000.0, base_lot=0.01, max_layers=3, shield_pct=4.0, tp_pips=25.0, sl_pips=15.0, grid_step=15.0, be_pips=10.0, be_lock=2.0, trail_start=15.0, trail_step=5.0):
        ...
        # High frequency / more trade entries configuration
        self.tp_pips = tp_pips
        self.sl_pips = sl_pips
        self.be_pips = be_pips
        self.be_lock_pips = be_lock
        self.trail_start_pips = trail_start
        self.trail_step_pips = trail_step
        self.grid_step_pips = grid_step

        self.pip_size = 0.0001
        self.pip_value = 10.0
        self.spread_val = SPREAD_PIP
    ...
    def fetch_historical_m15_data(self):
        print("Cargando datos M15 de EUR/USD para alta frecuencia...")
        df = yf.download('EURUSD=X', period='60d', interval='15m', progress=False)
        if isinstance(df.columns, pd.MultiIndex):
            df = df.xs('EURUSD=X', axis=1, level=1)
        df = df.dropna()
        return df

    def calculate_indicators(self, df):
        close = df['Close']
        df['EMA_Fast'] = close.ewm(span=8, adjust=False).mean()   # Faster 8 EMA
        df['EMA_Slow'] = close.ewm(span=21, adjust=False).mean()  # 21 EMA
        df['EMA_Trend'] = close.ewm(span=100, adjust=False).mean() # 100 EMA
    ...
    def run_backtest(self):
        df = self.fetch_historical_m15_data()
        ...
        for ym, month_df in df.groupby('YearMonth'):
            ...
            for idx, row in month_df.iterrows():
                ...
                high  = row['High']
                low   = row['Low']
                atr   = row['ATR'] if not np.isnan(row['ATR']) else (15.0 * self.pip_size)
                ...
                if not self.shield_hit and len(self.positions) == 0:
                    ...
                    # Extended active trading hours: 06:00 to 20:00 UTC
                    if 6 <= hour <= 20:
                        ema_f = row['EMA_Fast']
                        ema_s = row['EMA_Slow']
                        ema_t = row['EMA_Trend']
                        ...
                        if prev_idx >= 0:
                            ...
                            # Optimized high frequency breakout + momentum
    ...
    def manage_positions(self, close, high, low, atr, month_history):
        ...
        grid_dist = max(15.0 * self.pip_size, 1.0 * atr)
...
if __name__ == '__main__':
    sim = HighFrequencyEURUSDBacktester(
    ...
    m_reports = sim.run_backtest()

    print("=" * 105)
    print("      NUEVA CONFIGURACIÓN ALTA FRECUENCIA EUR/USD (M15 | EMAs 8/21 | RSI 48/52 | ATR Grid 15)")
    print("      RoboForex / Libertex ECN | Spread: 1.2 pips | Comisión: $3.50/lote")
    print("=" * 105)
    ...
    print(f"RESUMEN FINAL ALTA FRECUENCIA: Capital Final: ${sim.balance:.2f} | Beneficio Neto: {tot_sign}${tot_net:.2f} ({(tot_net/1000.0)*100:.2f}%)")
    print(f"Operaciones Totales: {tot_trades} trades | Win Rate Promedio Global: {np.mean([r['win_rate'] for r in m_reports]):.1f}%")
    print("=" * 105)
