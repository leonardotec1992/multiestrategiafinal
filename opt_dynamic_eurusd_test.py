...
class OptimizedDynamicEURUSDBacktester:
    def __init__(self, initial_balance=1000.0, base_lot=0.01, max_layers=3, shield_pct=4.0):
        ...
        # High Win-Rate & High Frequency Strategy Configuration
        self.tp_pips = 30.0
        self.sl_pips = 18.0
        self.be_pips = 10.0
        self.be_lock_pips = 2.0
        self.trail_start_pips = 18.0
        self.trail_step_pips = 6.0
        self.grid_step_pips = 18.0
    ...
    def fetch_m15_data(self):
        data = yf.download('EURUSD=X', period='60d', interval='15m', progress=False)
        if isinstance(data.columns, pd.MultiIndex):
            data = data.xs('EURUSD=X', axis=1, level=1)
        data = data.dropna()
        return data
    ...
    def calculate_indicators(self, df):
        ...
        df['EMA_Filter'] = close.ewm(span=50, adjust=False).mean()
    ...
    def run_backtest(self):
        ...
        for ym, month_df in df.groupby('YearMonth'):
            ...
            for idx, row in month_df.iterrows():
                ...
                high  = row['High']
                low   = row['Low']
                atr   = row['ATR'] if not np.isnan(row['ATR']) else (18.0 * self.pip_size)
                ...
                if not self.shield_hit and len(self.positions) == 0:
                    ...
                    # Peak London and New York Session Hours (07:00 to 18:00 UTC)
                    if 7 <= hour <= 18:
                        ema_f = row['EMA_Fast']
                        ema_s = row['EMA_Slow']
                        ema_m = row['EMA_Filter']
                        rsi   = row['RSI']
                        ...
                        if prev_idx >= 0:
                            ...
                            # High Probability Momentum Entry
                            buy_sig  = (ema_f > ema_s) and (prev_ema_f <= prev_ema_s) and (close > ema_m) and (rsi > 51.0)
                            sell_sig = (ema_f < ema_s) and (prev_ema_f >= prev_ema_s) and (close < ema_m) and (rsi < 49.0)
    ...
    def manage_positions(self, close, high, low, atr, month_history):
        ...
        grid_dist = max(18.0 * self.pip_size, 1.1 * atr)
...
if __name__ == '__main__':
    sim = OptimizedDynamicEURUSDBacktester(initial_balance=1000.0)
    m_reports = sim.run_backtest()

    print("=" * 105)
    print("      CONFIGURACIÓN OPTIMIZADA EUR/USD ALTA FRECUENCIA (M15 | EMA 9/21/50 | TP 30 / SL 18)")
    ...
    print(f"RESUMEN OPTIMIZADO EUR/USD: Capital Final: ${sim.balance:.2f} | Beneficio Neto: {tot_sign}${tot_net:.2f} ({(tot_net/1000.0)*100:.2f}%)")
    print(f"Operaciones Totales: {tot_trades} trades | Win Rate Promedio: {np.mean([r['win_rate'] for r in m_reports]):.1f}%")
    print("=" * 105)
